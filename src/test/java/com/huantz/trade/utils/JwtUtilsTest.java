...
class JwtUtilsTest {
  ...
  void testCreateAndVerifyWithRoles() {
    String token =
        JwtUtils.createToken(1L, List.of(AdminRoleEnum.SUPER_ADMIN), keyPair.getPrivate());
  ...
  void testCreateAndVerifyCustomClaims() {
    String token =
        JwtUtils.createToken(3L, Map.of("customKey", "customValue"), keyPair.getPrivate());
    assertThat(token).isNotBlank();

    Claims claims = JwtUtils.parseAndVerifyToken(token, keyPair.getPublic());
    assertThat(claims).containsEntry("customKey", "customValue");
  }
