...
class AccountControllerTest extends BaseControllerIntegrationTest {
  ...
  void testUnauthorized() {
    givenAnonymous().when().get("/admin/account/page").then().statusCode(401);
  }

  @Test
  @DisplayName("普通用户访问管理后台账号接口返回403")
  void testForbiddenForRegularUser() {
    givenUser(999L).when().get("/admin/account/page").then().statusCode(403);
  }

  @Test
  @DisplayName("管理员分页查询账号列表成功返回200")
  void testPageAccounts() {
    givenAdmin().when().get("/admin/account/page").then().statusCode(200).body(notNullValue());
  }
  ...
  void testAdminAccountLifecycle() {
    ...
    SellerContactDto seller =
        new SellerContactDto("张三", GenderEnum.MALE, "13800138000", "wx123456");
    ...
    // 1. 创建账号
    givenAdmin().body(createRequest).when().post("/admin/account").then().statusCode(202);
    ...
    // 3. 下架账号
    givenAdmin().when().put("/admin/account/{id}/unlist", accountId).then().statusCode(204);

    // 4. 上架账号
    givenAdmin().when().put("/admin/account/{id}/list", accountId).then().statusCode(204);

    // 5. 再次下架后删除账号
    givenAdmin().when().put("/admin/account/{id}/unlist", accountId).then().statusCode(204);

    givenAdmin().when().delete("/admin/account/{id}", accountId).then().statusCode(204);
  }
}
